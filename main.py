...
# Initialize the variable if they don't exist
...
# Function to ahsh passkey
def hash_passkey(passkey):
    return hashlib.sha256(passkey.encode()).hexdigest()

# Fuction to generate a key from passkey
...
if st.session_state.current_page == "Home":
    ...
    st.write("Use this app to **securely store and retrieve data** using unique encryption keys.")
    ...
    if st.button("Encrypt and Save"):
        if user_data and passkey and confirm_passkey:
            if passkey != confirm_passkey:
                ...
                st.info("Save this Data ID! You'll need it to retrieve data.")
        else:
        ...
        if data_id and passkey:
            if data_id in st.session_state.stored_data:
                ...
                if decrypted_text:
                    ...
                    st.error(f"Incorrect passkey! Attempte remaining: {3 - st.session_state.failed_attempts}")
            else:
                st.error("Data ID not found!")

            if st.session_state.failed_attempts >= 3:
                st.warning("Too many failed attempts! Redirect to Login Page.")
                st.session_state.current_page = "Login"
                st.rerun()
